...
# ---------------------------------------------------------------------------
# CSV-контекст: мета-блок и легенды таблиц
# ---------------------------------------------------------------------------

def _csv_meta(platform: str) -> str:
    ...
    return (
        ...
        "  ДРР от заказов (до СПП), % = (Реклама внутр. + Реклама внеш.) / Заказы до СПП, ₽ × 100\n"
        "  ДРР от продаж (до СПП), % = (Реклама внутр. + Реклама внеш.) / Продажи до СПП, ₽ × 100\n"
        "  Годовая доходность, % = Маржа до СПП, % / Оборот продаж, дни × 365\n"
        "  Ср.чек заказа до СПП, ₽ = Заказы до СПП, ₽ / Заказы, шт\n"
        "  Ср.чек продажи до СПП, ₽ = Продажи до СПП, ₽ / Продажи, шт\n"
        "  СПП = скидка постоянного покупателя WB (снижает выручку и маржу)\n"
        "\n"
        "ЧИСЛОВОЙ ФОРМАТ В ЭТОМ ФАЙЛЕ:\n"
        "  Проценты: X,X (запятая = десятичный разделитель), например 12,5 означает 12.5%\n"
        "  Остальные числа: целые (округлены), например 150000 означает 150 000 руб.\n"
        "\n"
        "ОБОЗНАЧЕНИЯ ФЛАГОВ:\n"
...
def _csv_legends(
    ...
    ad_pct: float = 20.0,
    ad_abs: float = 1000.0,
    anom_ratio: float = 0.3,
    anom_margin: float = 1000.0,
) -> dict:
    """Словарь {имя_таблицы: легенда} для CSV-экспорта."""
    is_wb = platform == "WB"
    return {
        "KPI": (
            "Итоговые показатели за оба периода.\n"
            "Строки: ключевые метрики (выручка, маржа, оборот, реклама и др.).\n"
            "Колонки: п1 (прошлый период), п2 (текущий), Δабс (изменение), Δ% (изменение в %).\n"
            "Приоритет анализа: Маржа ₽ → Годовая доходность, % → Маржа до СПП, %."
        ),
        "ТОП артикулов": (
            "ТОП-10 артикулов по Марже п2 (убывание).\n"
            "Только артикулы с положительной маржой п2.\n"
            "Используй для выявления лидеров."
        ),
        "Анти-ТОП артикулов": (
        ...
        "Масштабирование": (
            "Кандидаты на масштабирование — артикулы, прошедшие все фильтры роста (п2):\n"
            f"  Остатки ≥ {scale_stock} шт, Оборот продаж ≥ {scale_turn} дн и > 60 дн\n"
            f"  Запас дней (DaysCover) ≥ {scale_cover} дн\n"
            "  Маржа до СПП, % > 25%, Маржа ₽ > 0, ДРР от продаж < 5%\n"
            "  Статус ≠ Выводим / Архив\n"
            "Действие: увеличить закупку, поставку или рекламный бюджет."
        ),
        "ТОП моделей": (
            "Все модели, отсортированные по Марже п2 (убывание).\n"
            "SKU = количество уникальных артикулов в модели.\n"
            "Агрегация: числовые метрики суммируются; ср.чек и конверсии пересчитываются из агрегированных данных."
        ),
        "Реклама-заказы": (
            "Артикулы с существенным изменением рекламных расходов.\n"
            f"Флаг ⚠️: Δ рекламы ≥ {ad_pct:.0f}% И ≥ {ad_abs:.0f} ₽, но Δ заказов ≤ 0.\n"
            "ВАЖНО: лаг между ростом рекламы и ростом заказов составляет 7–14 дней.\n"
            "Если лаг прошёл, а заказы не выросли — реклама неэффективна."
        ),
        "Реклама-продажи": (
            "Артикулы с изменением ДРР и маржи.\n"
            "Флаг ⚠️: ДРР от продаж вырос И Маржа ₽ упала.\n"
            "ДРР растёт из-за: (а) роста рекламных расходов или (б) падения продаж. Проверяй оба фактора."
        ),
        "Склад": (
            "Артикулы с рисками заморозки капитала (только остатки ≥ 30 шт).\n"
            f"Флаг 🔴: Остатки п2 ≥ {wh_stock} шт И Оборот продаж п2 ≥ {wh_turn} дн.\n"
            "Действие при 🔴: распродажа, снижение цены, вывод или перемещение на другой склад."
        ),
        "Лаг цен": (
            "Расхождение между ср.чеком заказа и ср.чеком продажи (до СПП).\n"
            "Показываются только артикулы с разницей чеков ≥ 1% и ненулевыми ценами.\n"
            "Флаг ⚠️ Сильный лаг: расхождение > 10%.\n"
            "Причина лага: покупатели заказывают по одной цене, выкупают по другой (СПП-эффект или недавнее изменение цены)."
        ),
        "Цветовые аномалии": (
            "Аномалии маржинальности по (Color code, Коллекция, Модель).\n"
            f"Флаг ⚠️: маржа модели < {anom_ratio}× лучшей маржи в этом цвете+коллекции, при лучшей марже ≥ {anom_margin:.0f} ₽.\n"
            "Действие: проверить цены, скидки или позиционирование аномальной модели.\n"
            "Вопрос: почему один цвет работает, а другой — нет?"
        ),
        **({"Склейки WB": (
            "Анализ склеек WB (группа артикулов под одной карточкой).\n"
            "Роли:\n"
            "  донор — даёт ≥ 40% заказов или ≥ 40% продаж склейки\n"
            "  паразит — даёт < 10% продаж, но потребляет > 15% заказов (тянет трафик, не конвертирует)\n"
            "  каннибализация — флаг резкого перераспределения заказов между периодами\n"
            "Действие: убирать паразитов, усиливать доноров."
        )} if is_wb else {}),
        "Агр. по Модели": (
            "Агрегация всех метрик по Модели.\n"
            "Числовые метрики суммируются; ср.чек и конверсии пересчитываются из агрегированных сумм.\n"
            "Сортировка: по Марже п2 (убывание)."
        ),
        "Агр. Color+Коллекция": (
            "Агрегация всех метрик по (Color code, Коллекция).\n"
            "Числовые метрики суммируются; ср.чек и конверсии пересчитываются из агрегированных сумм."
        ),
        "Агр. по Статусу": (
            "Агрегация всех метрик по Статусу товара (активный, выводим, архив и др.).\n"
            "Числовые метрики суммируются; ср.чек и конверсии пересчитываются из агрегированных сумм."
        ),
        "Агр. по Склейке": (
            "Агрегация всех метрик по Склейке WB.\n"
            "Числовые метрики суммируются; ср.чек и конверсии пересчитываются из агрегированных сумм."
        ),
        "Воронка KPI": (
            "Итоговые показатели воронки WB (все артикулы).\n"
            "Воронка: Показы → Переходы (CTR) → Корзина (Conv2Cart) → Заказы (Conv2Order) → Выкупы (PurchaseRate).\n"
            "Формулы:\n"
            "  CTR, % = Переходы / Показы × 100\n"
            "  Conv2Cart, % = В корзину / Переходы × 100\n"
            "  Conv2Order, % = Заказали / Переходы × 100\n"
            "  PurchaseRate, % = Выкупили / Заказали × 100\n"
            "ВАЖНО: конверсии пересчитываются из суммарных данных, не усредняются."
        ),
        "Воронка×Экономика": (
            "Диагностические флаги по моделям: проблемы воронки + экономики.\n"
            "Флаги:\n"
            "  🔴 Реклама↑Трафик↓: расходы выросли, переходы упали → смени креативы/ключи\n"
            "  🟡 Цена↑Конверсия↓: цена после СПП выросла ≥ 50 ₽, Conv2Order упала → тест цены\n"
            "  🟠 Показы↓: показы упали ≥ 10% → проверь позицию в поиске, SEO, рекламу"
        ),
        "Точки роста конв.": (
            "Модели с высоким трафиком, но низкой конверсией — приоритеты для оптимизации.\n"
            "Типы проблем:\n"
            "  Проблема карточки/цены: высокие показы/переходы, но низкая Conv2Cart → улучшить карточку или снизить цену\n"
            "  Проблема заказа/доставки: высокая Conv2Cart, но низкая Conv2Order → проверить условия доставки\n"
            "  ✅ Контент/позиция работает: конверсии растут при стабильной цене → масштабировать"
        ),
        "Воронка по Модели": (
            "Агрегация воронки WB по Модели.\n"
            "Конверсии (CTR, Conv2Cart, Conv2Order, PurchaseRate) пересчитываются из суммарных данных."
        ),
        "Воронка по Статусу": (
            "Агрегация воронки WB по Статусу товара.\n"
            "Конверсии пересчитываются из суммарных данных."
        ),
        "Воронка по Склейке": (
            "Агрегация воронки WB по Склейке.\n"
            "Конверсии пересчитываются из суммарных данных."
        ),
        "WB_Ads_Summary": (
            "Итоговые показатели рекламы WB True Ads (данные из рекламного кабинета WB).\n"
            "ВАЖНО: True Ads CTR ≠ Funnel CTR.\n"
            "  Funnel CTR = все переходы на карточку с WB (поиск, рекомендации, реклама).\n"
            "  True Ads CTR = только клики из рекламного кабинета WB.\n"
            "Метрики: расходы, показы, клики, заказы, выручка, CPC (стоимость клика), CPO (стоимость заказа), ROAS, ДРР."
        ),
        "WB_Ads_ByProduct": (
            "Эффективность рекламы True Ads по артикулам (ТОП-30 по расходам п2).\n"
            "Используй для выявления неэффективных артикулов: высокий CPC/CPO при низком ROAS."
        ),
        "Ads×Funnel×Price": (
            "Совместный анализ: реклама + воронка + цена по моделям.\n"
            "Флаги:\n"
            "  ⚠️ Spend↑Orders↓: расходы выросли, заказы не выросли\n"
            "  ⚠️ CTR↓: кликабельность рекламы упала\n"
            "  ⚠️ CPC↑CR↓: стоимость клика выросла, конверсия упала"
        ),
        "Ads по Модели": (
            "Агрегация рекламы True Ads по Модели.\n"
            "Числовые метрики суммируются; CPC, CPO, ROAS пересчитываются из агрегированных данных."
        ),
        "Ads по Статусу": (
            "Агрегация рекламы True Ads по Статусу товара.\n"
            "Числовые метрики суммируются; CPC, CPO, ROAS пересчитываются из агрегированных данных."
        ),
        "Ads по Склейке": (
            "Агрегация рекламы True Ads по Склейке WB.\n"
            "Числовые метрики суммируются; CPC, CPO, ROAS пересчитываются из агрегированных данных."
        ),
        # Краткая сводка (Combined summary)
        "Общий итог WB + Ozon": (
            "Итоговые KPI по всем площадкам вместе (WB + Ozon).\n"
            "Строки: ключевые метрики. Колонки: п1, п2, Δабс, Δ%."
        ),
        "Итог по WB": (
            "Итоговые KPI только по WB.\n"
            "Строки: ключевые метрики. Колонки: п1, п2, Δабс, Δ%."
        ),
        "Итог по Ozon": (
            "Итоговые KPI только по Ozon.\n"
            "Строки: ключевые метрики. Колонки: п1, п2, Δабс, Δ%."
        ),
        "Модели WB": (
            "Все модели WB, отсортированные по Марже п2 (убывание).\n"
            "SKU = количество уникальных артикулов в модели."
        ),
        "Модели Ozon": (
            "Все модели Ozon, отсортированные по Марже п2 (убывание).\n"
            "SKU = количество уникальных артикулов в модели."
        ),
        "Воронка WB (общая)": (
            "Итоговые показатели воронки WB (все артикулы).\n"
            "Воронка: Показы → Переходы (CTR) → Корзина (Conv2Cart) → Заказы (Conv2Order) → Выкупы (PurchaseRate).\n"
            "Конверсии пересчитываются из суммарных данных, не усредняются."
        ),
    }
...
def _render_detailed(df: pd.DataFrame, platform: str, is_wb: bool,
    ...
    if tables_for_export:
        ...
        try:
            csv_bytes = export_brief_to_csv(
                tables_for_export,
                meta=_csv_meta(platform),
                legends=_csv_legends(
                    platform,
                    wh_stock=_wh_stock, wh_turn=_wh_turn,
                    scale_stock=_scale_stock, scale_turn=_scale_turn, scale_cover=_scale_cover,
                    ad_pct=_ad_pct, ad_abs=_ad_abs,
                    anom_ratio=_anom_ratio, anom_margin=_anom_margin,
                ),
            )
            st.download_button(
    ...
    if has_wb and has_ozon:
        ...
        if tables_for_export:
            ...
            try:
                csv_bytes = export_brief_to_csv(
                    tables_for_export,
                    meta=_csv_meta("WB + Ozon"),
                    legends=_csv_legends("WB"),
                )
